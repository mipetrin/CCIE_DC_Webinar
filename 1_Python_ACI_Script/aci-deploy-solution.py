...
def main():
    ...
    count = 1
    # FIX OBOE == would mean CCIE_DC_T26Z is missing
    while (count <= total_amount):
        if debug_enabled:
            print 'The count is:', count

        # FIX CCIE_DC_T1A ... CCIE_DC_T26Z // CCIE_DC_VRF1A ... CCIE_DC_VRF26Z
        tenant = aci.Tenant(name_prefix + "_T" + str(count) + str(chr(ord('@')+count)))
        context = aci.Context(name_prefix + "_VRF" + str(count) + str(chr(ord('@')+count)), tenant)
